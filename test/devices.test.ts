...
const sample = (timestamp: string) => ({ timestamp, hostname: "agent", os: "Test OS", cpuUsagePercent: 20, cpuTemperatureC: 40,storageTemperatureC:35,gpuTemperatureC:null, memory: { usedBytes: 1, totalBytes: 10, usedPercent: 10 }, filesystem: { usedBytes: 2, totalBytes: 10, usedPercent: 20 }, ipv4: "192.168.1.2", uptimeSeconds: 10 });
...
test("unconfigured remote device is offline without fixed host data", async () => {
  ...
  assert.equal(result.temperatures.gpu.currentC,null);
});

test("remote telemetry exposes all available temperature sensor values",async context=>{
  const original=globalThis.fetch;context.after(()=>{globalThis.fetch=original});
  globalThis.fetch=async()=>new Response(JSON.stringify(sample(new Date().toISOString())),{status:200});
  const result=await new RemoteDeviceProvider("test","TEST","http://agent").getStatus();
  assert.equal(result.temperatures.cpu.currentC,40);assert.equal(result.temperatures.storage.currentC,35);assert.equal(result.temperatures.gpu.currentC,null);
});

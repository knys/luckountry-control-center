...
export class DurableWorkItemRepository implements WorkItemRepository, ExecutionRepository,VerificationRepository {
  ...
  async completeVerification(id:string,result:VerificationResult):Promise<WorkItem|null>{return this.update(next=>{const lease=next.verification.leases.find(value=>value.verificationId===id&&value.status==="ACTIVE"),record=next.verification.records.find(value=>value.verificationId===id);if(!lease||!record)throw new DurableRepositoryError(`Active verification lease not found: ${id}`,this.databasePath);record.finishedAt=result.finishedAt;record.verifiedHead=result.verifiedHead??null;record.checks=structuredClone(result.checks);record.summary=result.summary;record.evidence=[...result.evidence];const decision=verificationDecision(record.criteria,result);record.status=decision.status;record.criterionEvidence=structuredClone(decision.criterionEvidence);record.summary=decision.reason;lease.status=decision.status==="ERROR"?"ABANDONED":"COMPLETED";if(decision.status==="ERROR")return null;const located=findItemLocation(next,record.workItemId);if(!located)throw new DurableRepositoryError(`WorkItem not found for verification: ${id}`,this.databasePath);const event=decision.status==="PASSED"?{type:"VERIFICATION_PASSED" as const}:decision.status==="FAILED"?{type:"VERIFICATION_FAILED" as const}:{type:"HUMAN_REQUIRED" as const,summary:decision.reason};const updated=transitionWorkItem(located.item,event).workItem;updated.evidence=[...updated.evidence,`verification:${id}:${decision.status}`,...result.evidence.slice(0,10)];located.data.workItems[located.index]=updated;return updated;});}

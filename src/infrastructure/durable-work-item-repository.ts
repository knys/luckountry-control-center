...
export class DurableWorkItemRepository implements WorkItemRepository, ExecutionRepository,VerificationRepository {
  ...
  async completeExecution(executionId: string, result: ExecutionResult, event: WorkEvent): Promise<WorkItem> {
    return this.update((next) => {
      ...
      const record = next.execution.records.find((item) => item.executionId === executionId)!; record.finishedAt = result.finishedAt; record.resultStatus = result.status; record.summary = result.summary; record.evidence = [...result.evidence];record.retryable=result.retryable;if(result.finalAgentMessage)record.finalAgentMessage=result.finalAgentMessage;if(result.baseHead)record.baseHead=result.baseHead;if(result.candidateBranch)record.candidateBranch=result.candidateBranch;if(result.candidateHead)record.candidateHead=result.candidateHead; return updated;
    });
  }

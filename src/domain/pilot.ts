...
export interface PilotControl { mode:AutomationMode;executionEnabled:boolean;verificationEnabled:boolean;scope:PilotScope|null;recoveryId:string|null;enabled:boolean;reason:string }
export type PilotCycleStatus="ARMED"|"EXECUTING"|"VERIFYING"|"VERIFIED"|"WAITING_HUMAN"|"FAILED"|"EXPIRED"|"CANCELLED";
export interface PilotCycle{cycleId:string;scopeFingerprint:string;workItemId:string;status:PilotCycleStatus;baseHead:string|null;candidateBranch:string|null;candidateHead:string|null;executionAttempts:number;verificationRuns:number;startedAt:string;updatedAt:string;reason:string;recoveryRequestId?:string|null;recoveryConsumedAt?:string|null;recoveryCount?:number;previousFailureReason?:string|null}

export function parsePilotControl(environment:NodeJS.ProcessEnv=process.env,now:number=Date.now()):PilotControl {
  const rawMode=environment.WORK_AUTOMATION_MODE?.trim().toLowerCase(),mode:AutomationMode=rawMode==="pilot"||rawMode==="production"?rawMode:"disabled",executionEnabled=flag(environment.WORK_EXECUTION_ENABLED),verificationEnabled=flag(environment.WORK_VERIFICATION_ENABLED),recoveryId=safeRecoveryId(environment.WORK_PILOT_RECOVERY_ID);
  if(mode!=="pilot")return{mode,executionEnabled,verificationEnabled,scope:null,recoveryId,enabled:false,reason:mode==="production"?"production automation is reserved":"automation is disabled"};
  ...
  if(Date.parse(scope.expiresAt)<=now)return{mode,executionEnabled,verificationEnabled,scope,recoveryId,enabled:false,reason:"pilot scope is expired"};
  if(!executionEnabled||!verificationEnabled)return{mode,executionEnabled,verificationEnabled,scope,recoveryId,enabled:false,reason:"both execution and verification flags are required"};
  return{mode,executionEnabled,verificationEnabled,scope,recoveryId,enabled:true,reason:"pilot is armed"};
}
...
function safeRecoveryId(value:string|undefined):string|null{const id=value?.trim();return id&&/^[A-Za-z0-9._-]{1,200}$/.test(id)?id:null;}
function disabled(mode:AutomationMode,executionEnabled:boolean,verificationEnabled:boolean,reason:string):PilotControl{return{mode,executionEnabled,verificationEnabled,scope:null,recoveryId:null,enabled:false,reason};}

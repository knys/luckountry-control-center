...
async function gpuTemperature(): Promise<number | null> {
  try {
    const { stdout } = await execFileAsync("/usr/bin/nvidia-smi", ["--query-gpu=temperature.gpu", "--format=csv,noheader,nounits"], { timeout: 2000 });
    const values = stdout.split(/\s+/).map(Number).filter(Number.isFinite);

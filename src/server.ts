...
import { DurableTemperatureHistory } from "./temperature-history.js";
...
const temperatureHistory=await DurableTemperatureHistory.open(process.env.TEMPERATURE_HISTORY_PATH??join(commissionData,"temperature-history.jsonl"));
const deploymentToken=randomBytes(32).toString("hex"),deploymentTokenPath=process.env.COMMISSION_DEPLOY_TOKEN_PATH??join(commissionData,"commission-deploy-token");await writeFile(deploymentTokenPath,`${deploymentToken}\n`,{mode:0o640});await chmod(deploymentTokenPath,0o640);const requestDeployment=()=>setTimeout(()=>{const child=spawn("/usr/bin/sudo",["-n","/usr/local/libexec/lcc-tx-maintenance","TX_LCC_DEPLOY_DISABLED","knys/luckountry-control-center"],{stdio:"ignore",detached:true,shell:false});child.unref()},25);

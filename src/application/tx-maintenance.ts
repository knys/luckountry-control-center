...
export function reconcilePilotCycle(cycle:PilotCycle,execution:ExecutionState,verification:VerificationState,workState:string,now:string):PilotCycle{
  ...
  const status=workState==="FAILED"?"FAILED":workState==="WAITING_HUMAN"?"WAITING_HUMAN":workState==="VERIFYING"?"VERIFYING":workState==="RUNNING"||workState==="RETRYING"?"EXECUTING":verified?.status==="PASSED"?"WAITING_HUMAN":cycle.status;

...
import { CommissionInbox,type WatcherStatus } from "./commission-inbox.js";
import { BoundedLocalCodexExecutor,DurableCommissionRunRegistrar,commissionExecutors } from "./commission-runs.js";
...
async function save(x:WatcherStatus){await mkdir(dirname(statusPath),{recursive:true});const tmp=`${statusPath}.${process.pid}.tmp`;await writeFile(tmp,JSON.stringify(x,null,2)+"\n");await rename(tmp,statusPath)}
...
async function main(){let failures=0;while(!stopping){const now=new Date(),next=new Date(now.getTime()+interval).toISOString();try{
  ...
  const item=queued.sort((a,b)=>b.priority-a.priority||a.detectedAt.localeCompare(b.detectedAt))[0];
  if(!item){await save({state:"WATCHING",lastScan:now.toISOString(),nextScan:next,queuedCount:0,activeCount:0,humanWaitingCount:waiting,currentWorkItem:null,currentActor:null,failure:null});failures=0;await wait(interval);continue}
  await save({state:"DISPATCHING",lastScan:now.toISOString(),nextScan:null,queuedCount:queued.length,activeCount:0,humanWaitingCount:waiting,currentWorkItem:item.id,currentActor:null,failure:null});
  const orchestrator=new SelfCommissioningOrchestrator(store,commissionExecutors(executor)),dispatcher=new PersistentCodexDispatcher(store,orchestrator,{report:async()=>undefined});dispatcher.start(item.runId!);for(let i=0;i<100;i++){const run=await store.get(item.runId!);if(run?.status==="RUNNING")break;await new Promise(r=>setImmediate(r))}
  const active=await store.get(item.runId!);await save({state:active?.status==="RUNNING"?"RUNNING":"DISPATCHING",lastScan:new Date().toISOString(),nextScan:null,queuedCount:Math.max(0,queued.length-1),activeCount:active?.status==="RUNNING"?1:0,humanWaitingCount:waiting,currentWorkItem:item.id,currentActor:active?.activeActor??null,failure:null});await dispatcher.drain();failures=0;
}catch(e){failures++;const delay=Math.min(interval*2**failures,900_000);await save({state:"DEGRADED",lastScan:new Date().toISOString(),nextScan:new Date(Date.now()+delay).toISOString(),queuedCount:0,activeCount:0,humanWaitingCount:0,currentWorkItem:null,currentActor:null,failure:String(e).slice(0,500)});await wait(delay)}}}
function stop(){stopping=true;sleeper.cancel();executor.stop()}
process.on("SIGTERM",stop);process.on("SIGINT",stop);await main();

...
try {
  ...
  const restoredStore=await DurableSelfCommissioningStore.open(storePath);assert.equal((await restoredStore.list())[0]?.status,"WAITING_HUMAN");assert.deepEqual(calls.slice(0,6),["INSPECT_CURRENT_STATE","TX_LCC_RECONCILE","ASSERT_REMEDIATION_ELIGIBILITY","ENABLE_BOUNDED_PILOT","CODEX_JOB","RECOVER_TIMED_OUT_CANDIDATE"]);assert.equal(calls.filter(value=>value==="INSPECT_CURRENT_STATE").length,1);

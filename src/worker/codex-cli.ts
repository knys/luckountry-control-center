...
export async function executeCodex(request: ExecutionRequest, workspace: WorkspaceConfig, probe: CodexProbe, run: CommandRunner = runCommand, signal?:AbortSignal): Promise<ExecutionResult> {
  ...
  return { executionId: request.executionId, status: output.code === 0 && valid ? "SUCCEEDED" : "FAILED", startedAt, finishedAt, exitCode: output.code, summary, evidence: lines.slice(-10).map(line => sanitizeDiagnostic(line,500)), retryable: output.code !== 0 };
}
...
export function sanitizeDiagnostic(value:string,limit=DIAGNOSTIC_TAIL_LIMIT):string{return value.replace(/(token|secret|api[_-]?key)\s*["']?\s*[:=]\s*["']?[^\s,"'}]+/gi,"$1=[REDACTED]").replace(/\bBearer\s+\S+/gi,"Bearer [REDACTED]").replace(/\b(?:sk|gh[opusr])-[A-Za-z0-9_-]{8,}\b/g,"[REDACTED]").slice(-limit);}

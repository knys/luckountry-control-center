...
export class WorkerExecutionStore {
  ...
  async patch(id: string, change: (record: WorkerExecutionRecord) => void): Promise<WorkerExecutionRecord> { return this.update(next => { const record = next.executions.find(item => item.executionId === id); if (!record) throw new Error("execution not found"); change(record); record.summary = record.summary.slice(0, 500); record.evidence = record.evidence.slice(0, 10).map(item => item.slice(0, 500)); return record; }); }

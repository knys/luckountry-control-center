...
import { TemperatureHistoryStore, summarizeTemperature, type TemperatureHistory } from "./temperature-history.js";
...
function status(id: string, name: string, telemetry: AgentTelemetry, now: number, thresholds: DeviceThresholds): DeviceStatus {
  const current={cpu:telemetry.cpuTemperatureC,storage:telemetry.storageTemperatureC,gpu:telemetry.gpuTemperatureC};
  return { id, name, hostname: telemetry.hostname, status: stateFor(telemetry, now, thresholds), os: telemetry.os, cpuUsagePercent: telemetry.cpuUsagePercent, cpuTemperatureC: telemetry.cpuTemperatureC, storageTemperatureC:telemetry.storageTemperatureC,gpuTemperatureC:telemetry.gpuTemperatureC,temperatures:summarizeTemperature([],current,telemetry.timestamp), memory: telemetry.memory, filesystem: telemetry.filesystem, ipv4: telemetry.ipv4, uptimeSeconds: telemetry.uptimeSeconds, lastSeen: telemetry.timestamp };
}
...
export class RemoteDeviceProvider implements DeviceProvider {
  ...
  async getStatus(): Promise<DeviceStatus> {
    ...
    return { id: this.id, name: this.name, hostname: null, status: "OFFLINE", os: null, cpuUsagePercent: null, cpuTemperatureC: null,storageTemperatureC:null,gpuTemperatureC:null,temperatures:summarizeTemperature([],{},new Date().toISOString()), memory: emptyCapacity(), filesystem: emptyCapacity(), ipv4: null, uptimeSeconds: null, lastSeen: null };
  }
}
...
export class WorkerDeviceProvider implements DeviceProvider {
  ...
  async getStatus():Promise<DeviceStatus>{const descriptor=await this.registry.get(this.workerId),online=descriptor?.status==="ONLINE",timestamp=new Date().toISOString();return{id:this.id,name:"GTX1060 PC",hostname:online?"GTX1060":null,status:online?"ONLINE":"OFFLINE",os:online?`LCC Worker ${descriptor.agentVersion??"unknown"} / ${descriptor.codexVersion??"Codex unavailable"}`:null,cpuUsagePercent:null,cpuTemperatureC:null,storageTemperatureC:null,gpuTemperatureC:null,temperatures:summarizeTemperature([],{},timestamp),memory:emptyCapacity(),filesystem:emptyCapacity(),ipv4:online?new URL(this.baseUrl).hostname:null,uptimeSeconds:null,lastSeen:online?timestamp:null};}
}

class RecordingDeviceProvider implements DeviceProvider {
  get id(){return this.provider.id}
  constructor(private readonly provider:DeviceProvider,private readonly history:TemperatureHistoryStore){}
  async getStatus():Promise<DeviceStatus>{const result=await this.provider.getStatus(),timestamp=result.lastSeen??new Date().toISOString();result.temperatures=await this.history.record(result.id,timestamp,{cpu:result.cpuTemperatureC,storage:result.storageTemperatureC,gpu:result.gpuTemperatureC});return result;}
}
...
export function createDeviceProviders(environment: NodeJS.ProcessEnv = process.env): DeviceProvider[] {
  ...
  const telemetryUrl=environment.DEVICE_GTX1060_URL?.trim();
  const gtx=telemetryUrl?new RemoteDeviceProvider("gtx1060","GTX1060 PC",telemetryUrl):workerId&&workerUrl&&keyId&&secret?new WorkerDeviceProvider(new RemoteWorkerRegistry([{workerId,baseUrl:workerUrl,credentials:{keyId,secret}}]),workerId,workerUrl):new RemoteDeviceProvider("gtx1060", "GTX1060 PC", undefined);
  ...
  const history=new TemperatureHistoryStore(environment.TEMPERATURE_HISTORY_PATH?.trim()||"/var/lib/luckountry-control-center/temperature-history.json");
  return providers.map(provider=>new RecordingDeviceProvider(provider,history));
}

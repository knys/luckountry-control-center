...
export interface V2Job{
  ...
  prUrl:string|null;ciStatus:"PENDING"|"PASS"|"FAIL"|null;mainSha:string|null;deployedSha:string|null;healthStatus:"PENDING"|"PASS"|"FAIL"|null;deploymentEvidence:string|null;legacyDeploymentUnverified:boolean;humanGate:HumanGate|null;failure:string|null;
...
export function assertJob(job:V2Job){
  ...
  if(job.state==="COMPLETED"&&job.repository==="knys/luckountry-control-center"&&!job.legacyDeploymentUnverified&&(job.healthStatus!=="PASS"||job.deployedSha!==job.mainSha))throw new Error("LCC COMPLETED requires healthy deployed main SHA");
}
